...
@Component({
  selector: 'app-temp-list-reject',
  templateUrl: './temp-list-reject.component.html',
  styleUrls: ['./temp-list-reject.component.css']
})
export class TempListRejectComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.templateUrl = 'BranchCreation/TemplateController/fetchRejectedRecordForUpdation';
  ...
  editTempDetails(info: any): void {
    ...
    this.smsService.checkTemplatePresentForApproval(info.templateId).subscribe(resdata => this.responceData = resdata, reserror => this.errorMsg = reserror, () => {
      if (this.responceData.status == '200') {
        ...
        sessionStorage.setItem('edit_temp_' + info.templateModId, infoValue);
        this.router.navigate(['/sms/edit-template', info.templateModId]);
